...
VWFDrawCharacterInternal::
  ; de is the pointer to the drawing area
  ld a, e
  ld [W_VWFTileDst], a
  ld a, d
  ld [W_VWFTileDst+1], a
...
.loop_tiles
  ...
  ; Shift the current letter as many pixels as we've already written in the current tile
.do_shift
  and a
  jr z, .done_shift
  srl d
...
.do_shift_left
  ...
  jr .do_shift_left
.done_shift_left
  xor a ; There's no tile to mix with here
  ld [hli], a
  ld a, d  
  ld [hli], a
